...
$name = mysqli_real_escape_string($connect, $namehelper );
 ...
 $sql = "SELECT SUM(Stok_Alis_Tutari) AS Stok_Alıs , UR.Markasi AS Markasi,SUM(KDV_Haric_Ciro) AS Ciro 
 ...
 $resultTable = mysqli_query($connect, $sql);
  ...
  $sqlTwo = "SELECT SUM(Stok_Alis_Tutari) AS Stok_Alıs , UR.Markasi AS Markasi,SUM(KDV_Haric_Ciro) AS Ciro
					FROM pharmana_Hareket_db.general_Table HAR, pharmana_urun_db.general_Table UR
 ...
 $resultTableTwo = mysqli_query($connect, $sqlTwo);
												...
												foreach($resultSet as $tyc){
														foreach($resultSetTwo as $tylc){
														  if($tyc['Markasi'] === $tylc['Markasi']) {
															...
															$Stok_Devir_Günü = ($tyc["Stok_Alıs"] / ( $tyc["Ciro"] / 25));
															$Stok_Devir_Günü_LastYear = ($tylc["Stok_Alıs"] / ( $tylc["Ciro"] / 25));
															 if($tyc["Stok_Alıs"]=="")
																  $tyc["Stok_Alıs"] = "0.00";
															  if($tyc["Ciro"]=="")
																  $tyc["Ciro"] ="0.00";
															...
															echo '  "Stok_Alıs": "' . number_format($tyc["Stok_Alıs"],2) . '",' . "\n";
															echo '  "Satıs": "' . number_format($tylc["Ciro"],2) . '",' . "\n";
															echo '  "StokGunuThisYear": "' . number_format($Stok_Devir_Günü,2) . '",' . "\n";
															echo '  "StokGunuLastYear": "' . number_format($Stok_Devir_Günü_LastYear,2). '"' . "\n";
															echo " }";
															$prefix = ",\n";
															$i++;
															$variable=false;
												...
												foreach($resultSetTwo as $tyc){
														...
														if ($variable==true)
														{
															if($tyc["Ciro"]=="")
																  $tyc["Ciro"] ="0.00";
															$Stok_Devir_Günü_LastYear = ($tyc["Stok_Alıs"] / ( $tyc["Ciro"] / 25));
															echo $prefix . " {\n";
															echo '  "Sira": "' . $i . '",' . "\n";
															echo '  "Markasi": "' . $tyc["Markasi"] . '",' . "\n";
															echo '  "Stok_Alıs": "' . '0.00' . '",' . "\n";
